$(function() {
  $(".switcher").on("change", function() {
    ...
    $switchers.each(function(index, elem) {
      $(elem).prop("disabled", false).removeClass("c_disabled").closest(".uno_part_wrapper").find(".option_overlay").remove();
    });
    
    if($disables) {
      for(i = 0; i < $disables.length; i++) {
        var elem_to_disable = $("[data-option-id='" + $disables[i] + "']");
        elem_to_disable.prop("disabled", true).addClass("c_disabled").closest(".part_option_wrapper").prepend("<div class='option_overlay'/>");
      }
    }
