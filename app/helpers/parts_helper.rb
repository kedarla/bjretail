module PartsHelper
  def options_for_part_child(part_child)
  	...
  	part_child.options.each do |o|
  	  options_str += "<option value='#{o.id}' data-option-subid='option_subid_#{o.id}', data-option-name='#{o.name}', data-option-id='#{part_child.id}' #{o.is_default? ? 'selected' : ''}>#{o.name}</option>"
  	end
